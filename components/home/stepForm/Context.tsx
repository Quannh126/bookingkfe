...
type ContextProps = {
    ...
    // eslint-disable-next-line no-unused-vars
    handleSeleteSeat(value: Array<Number>, fare: Number): void;
    handleNext(): void;
    handleBack(): void;
    variant: Variant;
    margin: Margin;
...
export function StepsProvider({ children }: ProviderProps) {
    ...
    // Handle form change
    const handleSeleteSeat = useCallback(
        (value: Array<Number>, fare: Number) => {
            console.log("test");
            dispatch({
                type: "form-value",
                name: "selected_seats",
            ...
            dispatch({ type: "form-value", name: "fare", fieldValue: fare });
        },
        []
    );
                ...
                switch (validate) {
                    case "text":
                        if (value && !isText.test(value))
                            error =
                                helperText || "This field accepts text only.";
                        break;
